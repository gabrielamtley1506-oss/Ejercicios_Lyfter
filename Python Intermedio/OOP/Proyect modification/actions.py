...
def display_all_students(students_list): 
    ...
    for i, student in enumerate(students_list, 1):
        ...
        print(f"   Social Grade: {student.social_grade}")
...
def top_3_students(students_list):
    ...
    for i, (student, avg) in enumerate(top_3, 1):
        print(f"\n{i}. {student.name} ({student.section})")
        print(f"   Average: {avg}")
...
def delete_student(students_list):
    ...
    for student in students_list:
        if student.name.lower() == name.lower() and student.section.lower() == section.lower():
            ...
            print(f"\nStudent '{student.name}' from section '{student.section}' deleted successfully!")
            return
...
def failed_students(students_list):
    ...
    for i, student in enumerate(failed, 1):
        avg = average(student)
        print(f"\n{i}. Name: {student.name} ({student.section})")
        print(f"   Spanish Grade: {student.spanish_grade}")
        print(f"   English Grade: {student.english_grade}")
        print(f"   Social Grade: {student.social_grade}")
        print(f"   Science Grade: {student.science_grade}")
        ...
        if student.social_grade < 60:
            failed_subjects.append(f"Social ({student.social_grade})")
